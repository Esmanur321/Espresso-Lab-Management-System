...
<template>
  <div class="corporate-slider" @mouseenter="stopTimer" @mouseleave="startTimer">
    <div class="arrow-wrapper left"><SliderArrow direction="left" @click="prevSlide" /></div>

    <div class="slider-image-wrapper">
      <img :src="currentImage" :alt="currentSlide.alt || 'Banner'" class="full-banner-image" />
    </div>
...
.corporate-slider {
  position: relative;
  width: 100%;
  height: 100%; /* Ebeveynin (HomePage) yüksekliğini doldurur */
  overflow: hidden;
...
.full-banner-image {
  width: 100%;
  height: 100%;
  object-fit: fill; /* Resmi tam ekrana yay/esnet */
  display: block;
}
...
.left { left: 30px; }
.right { right: 30px; }
.dots-navigation { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); z-index: 10; }
</style>